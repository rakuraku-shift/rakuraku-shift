/* ══════════════════════════════════════════
   RAKURAKU 求人自動入力 — シェアフル content script
   ══════════════════════════════════════════ */
...
(function() {
  async function main() {
    ...
    if (!looksLikePostingPage()) {
      showBanner('シェアフル', '求人作成ページに移動してから内容を確認してください。', true);
      return;
    }
    fillForm(pending);
    showBanner('シェアフル', '求人内容を自動入力しました。確認して公開してください。', false);
  }
  ...
  function fillForm({ jobText, jobTitle, wage }) {
    fillFirstMatch([
      ...
      'input[placeholder*="タイトル"]', 'input[placeholder*="求人名"]',
      'input[id*="title" i]',
    ], jobTitle);

    fillFirstMatch([
      'textarea[name*="description" i]', 'textarea[name*="content" i]',
      'textarea[name*="detail" i]', 'textarea[placeholder*="詳細"]',
      'textarea[placeholder*="仕事内容"]', 'textarea',
    ], jobText);

    fillFirstMatch([
      'input[name*="wage" i]', 'input[name*="hourly" i]', 'input[name*="salary" i]',
      'input[type="number"][placeholder*="時給"]', 'input[type="number"][placeholder*="円"]',
    ], String(wage || ''));
  }
  ...
  function getPending() {
    return new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_PENDING' }, resolve));
  }
  ...
  main().catch(e => console.error('[RAKURAKU シェアフル]', e));
})();
